...
lib2 = CDLL("/home/dcs/workspace/dcs-master/FowlerCalculation/libsampling_cal.so")
...
class DC():
    def __init__(self, gui=False):
        ...
        self.ics_password = cfg.get('ICS', 'password')
    ...
    def AcquireRamp(self):
        ...
        #2022-09-05 changed buf -> buf_size
        #arr_list = []
        #arr = np.array(arr_list)
        #buf = arr.ctypes.data_as(POINTER(c_int))
        buf_size = POINTER(c_int)

        sts = lib.MACIE_ConfigureGigeScienceInterface(self.handle, self.slctMACIEs, 0, frameSize, 42037, buf_size)  # 0-16bit
        if sts != MACIE_OK:
            msg = "Science interface configuration failed. buf = %d" % buf_size
            self.logwrite(BOTH, msg)
            return False
        msg = "Science interface configuration succeeded. buf (KB) = %d" % buf_size
        self.logwrite(BOTH, msg)
    ...
    def StopAcquisition(self):
        ...
        if lib.MACIE_WriteASICReg(self.handle, self.slctASICs, ASICAddr_State, 0x8000, self.option) != MACIE_OK:
    ...
    def WriteFitsFile(self):
        ...
        res = lib2.fowler_calculation(self.samplingMode, self.reads, idx, data)
    ...
    def save_fitsfile_sub(self, idx, filename, cur_datetime, ramp, group, read):
        ...
        #-------------------------------------------------------------------------------------
        #temperature
        with open(g_exe_path+"/HK_db.json", 'r') as f:
...
if __name__ == "__main__":
    ...
    local_channel = server.connect_to_server('localhost', g_dcs_id, g_dcs_pwd)

    # as producer
    server.define_producer(local_channel, EX_CORE, 'direct')
    
    # as consumer
    core_queue = server.binding(local_channel, EX_GUI, 'direct', QUE_CORE)

    def callback(ch, method, properties, body):
        print('[core <- gui] ' + body.decode())
        
        cmd = body.decode()
        if cmd == 'LibVersion':
            ver = cmd + ':' + dc.LibVersion()
            ch.basic_publish(exchange=EX_CORE, routing_key=QUE_GUI, body=ver)
            print('[core -> gui] ' + ver)               

    def consumer():
        try:
            local_channel.basic_consume(queue=core_queue, on_message_callback=callback, auto_ack=True)
    ...
    def send_message(_rounting_key, msg):
        local_channel.basic_publish(exchange=EX_CORE, routing_key=_rounting_key, body=msg)
        message = '[core -> %s] %s' % (_rounting_key, msg)
        print(message)

    #send_message(QUE_GUI, 'core start!')
...
'''    
    ...
    dc.samplingMode = 1 #0~3

    #dc.fowlerTime = 1.63 in header??
    #dc.expTime = ???
    if dc.samplingMode == 0:
        dc.SetRampParam(1, 1, 1, dc.fowlerTime, 1)
    elif dc.samplingMode == 1:
        dc.SetFSParam(1, 1, 1, dc.fowlerTime, 1)
    elif dc.samplingMode == 2:
        dc.SetFSParam(1, 1, 1, dc.fowlerTime, 2)
    elif dc.samplingMode == 3:
        dc.SetFSParam(1, 16, 1, dc.fowlerTime, 1)
...
'''
'''
    lastfilename = g_exe_path + "/Data/CDS/20220602_090341/H2RG_R01_M01_N02.fits"
    fullpath = g_exe_path + "/Data/CDS/20220602_090341/Result/"
